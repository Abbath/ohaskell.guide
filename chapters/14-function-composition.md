...
Виходить такий собі конвеєр: на вході рядок з IP-адресою, на виході &mdash; повідомлення у нашому терміналі. Існує інший спосіб поєднання двох функцій воєдино.

## Композиція і застосування
...
Оператор композиції об'єднує дві функції воєдино (або компонує їх, англ. compose). Коли ми пишемо:
...
Вона показує наш рух з точки `A` точку `B`. Тому часто говорять про &laquo;функції з `A` в `B`&raquo;. Так, про функції `checkLocalhost` можна сказати як про &laquo;функцію з `String` в `String`&raquo;.
...
Тобто композиція правоассоціативна (англ. right-associative): спочатку застосовується функція праворуч, а потім &mdash; ліворуч.
...
Краса композиції в тому, що компонувати ми можемо скільки завгодно функцій:
...
Стривайте, але де ж ім'я аргументу? А його більше немає, воно нам не потрібно. Адже ми знаємо, що застосування функції можна легко замінити внутрішнім виразом функції. А раз так, вираз `logWarn` може бути замінено на вираз `warning . correctSpaces . asciiOnly`. Зробимо ж це:
...
Заради справедливості слід зауважити, що не всі Haskell-розробники люблять позбавлятися від круглих дужок, деякі вважають за краще використовувати саме їх. Що ж, це лише питання стилю та звичок.
...
Ось так і відбувається &laquo;об'єднання&raquo; двох функцій: ми просто повертаємо ЛФ від одного аргументу, всередині якої правоассоціативно викликаємо обидві функції. А аргументом в даному випадку є той самий рядок з IP-адресою:
