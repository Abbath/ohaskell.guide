...
Ось знову ці маленькі літери! Пам'ятаєте, я обіцяв розповісти про них? Розповідаю: малою літерою прийнято іменувати поліморфний (англ. polymorphic) тип. Поліморфізм &mdash; це різноманітність, многоформенність. В даному випадку мова йде не про зазначення конкретного типу, а про &laquo;типову заглушку&raquo;. Ми говоримо: &laquo;Функція `map` застосовується до функції з якогось типу `a` в якийсь тип `b` і до списку типу `[a]`, а результат її роботи &mdash; це інший список типу `[b]`&raquo;. Типовими заглушками я назвав їх тому, що на їх місце стають конкретні типи, що робить функцію `map` дуже гнучкою. Наприклад:
...
Функція `map` застосовується до двох аргументів: до функції `toUpper` і рядка `str`. Функція `toUpper` із стандартного модуля `Data.Char` переводить символ типу `Char` верхній регістр:
...
Все встало на свої місця. Функція `map` в даному випадку бере функцію `toUpper` і біжить по списку, послідовно застосовуючи цю функцію до його елементів:
...
Функція `toStr` працює вже зі списками різних типів: на вході список чисел з плаваючою точкою, на виході список рядків. При запуску цієї програми ми побачимо наступне:
...
В даному випадку, раз вже ми працюємо з числами типу `Double`, тип функції `show` такий:
...
Функція `map` застосовує функцію `show` до чисел з першого списку, на виході отримуємо другий список, вже з рядками. І як і у випадку з `toUpper` функція `show` нічого не підозрює про те, що нею оперували в якості аргументу функції `map`.
...
Функція `map` очікує два аргументи, це відображено в її типі. Але що буде, якщо застосувати її не до двох аргументів, а лише до одного? В цьому випадку відбудеться ще одне &laquo;магічне&raquo; перетворення, яке називається частковим застосуванням (англ. partial application) функції. Частковим називають таке застосування, коли аргументів менше ніж очікується.
...
Функція `map` отримала лише перший аргумент, а де ж другий? Другий, як ми вже знаємо, що буде одержаний нею вже потім, після того, як ми підставимо цей вираз на місце функції `ten`. Але що ж відбувається з функцією `map` до цього? А до цього з нею відбувається часткове застосування. Зрозуміло, що вона ще не може виконати свою роботу, тому, будучи застосованою лише до одного аргументу, вона повертає ЛФ! Зіставимо з типом функції `map`, і все встане на свої місця:
...
Оскільки голова у вигляді першого аргументу типу `(a -> b)` вже дана, залишилося отримати другий аргумент. Тому ЛФ, породжена частковим застосуванням, очікує єдиний аргумент, яким і буде той самий другий, а саме список`[1.2, 1,4, 1.6]`.
...
Це оголошення функції `replace`, приймаючої три рядки: перша містить те, що шукаємо, друга містить те, на що замінюємо, а в третій лежить те, де шукаємо. Наприклад:
