...
лямбда-вираз  аргумент
```

Лямбда-вираз породжує тимчасову функцію, яку ми відразу ж застосовуємо до аргументу `5`. ЛФ з одним аргументом ще називають &laquo;ЛФ від одного аргументу&raquo; чи &laquo;ЛФ одного аргументу&raquo;.

## Будова

Будова гранично проста:
...
Ми можемо ввести проміжне значення для ЛФ:
...
Тут вираз `mul` оголошується рівним ЛФ, і тепер ми можемо застосовувати `mul` так само, ніби це лямбда-вираз:
...
Ми знаємо, що у всіх даних в Haskell-програмі обов'язково є якийсь тип, який перевіряється на етапі компіляції. Питання: який тип у виразу `mul` з попереднього прикладу?

```haskell
where mul = \x y -> x * y -- Який тип?
```

Відповідь проста: тип `mul` такий же, як і у цієї ЛФ. З цього ми робимо важливий висновок: ЛФ має тип, як і звичайні дані. Але якщо ЛФ є функцією (просто гранично лаконічною) &mdash; значить і у звичайної функції теж є тип!

В імперативних мовах між функціями і даними проведена чітка межа: ось це функції, а ось те &mdash; дані. Проте в Haskell між даними і функціями різниці немає, адже і те й інше спочиває на одній і тій же Черепасі. Ось тип функції `mul`:

```haskell
mul :: a -> a -> a
```

Почекайте, скажете ви, але ж це оголошення функції! Абсолютно вірно: оголошення функції &mdash; це і є вказівка її типу. Пам'ятаєте, коли ми вперше познайомилися з функцією, я уточнив, що її оголошення розділене подвійним двокрапкою? Так ось це подвійна двокрапка і являє собою вказівку типу:
...
І раз функція характеризується типом так само, як і інші дані, ми робимо ще одне важливе відкриття: функціями можна оперувати як даними. Наприклад, можна створити список функцій:
...
Раз вже між ЛФ і простими функціями фактично немає розходжень, а функції є частковим випадком даних, ми можемо створювати функції локально для інших функцій:
...
Тепер вираження `containsAtSign` і `endsWithCom` прирівняні до ЛФ від одного аргументу. У цьому випадку ми звичайно ж не вказуємо тип цих виразів. Втім, якщо дуже хочеться, можна вказати, наприклад:

```haskell
containsAtSign = (\e -> "@" `isInfixOf` e) :: String -> Bool

                  вираз, що породжує          тип цієї функції
                  функцію
```

Лямбда-вираз взято в дужки, щоб оголошення типу відносилося до функції в цілому, а не тільки до аргументу `e`:
...
Втім, на практиці вказівка типу ЛФ зустрічається вкрай рідко, бо не потрібно.

Тепер, познайомившись з ЛФ, ми будемо періодично їх використовувати.

І наостанок, питання. Пам'ятайте тип функції `mul`?
...
Йшли 30-ті роки минулого століття, комп'ютерів не було, і всі наукові роботи набиралися на друкарських машинках. У первісному варіанті, щоб виділяти ім'я аргументу ЛФ, Черч ставив над ім'ям аргументом символ, схожий на `^`. Але коли він здавав роботу набірнику, то згадав, що друкарська машинка не зможе відтворити такий символ над буквою. Тоді він виніс цей &laquo;дашок&raquo; перед ім'я аргументу, і вийшло щось накшталт:
