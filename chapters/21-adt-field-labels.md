...
Більше того, коли ми дивимося на визначення типу, призначення полів залишається таємницею за сімома печатями. Бачите передостаннє поле? Воно має тип `Bool` і, ясна річ, відображає якийсь прапорець. Але що це за прапорець, читач не уявляє. На щастя, існує спосіб, який рятує нас від обох цих проблем.
...
Якщо два чи більше полів одного типу йдуть поспіль, тип можна вказати лише для останньої з міток. А отже, якщо у нас є ось такий тип:
...
Поля `firstName`, `lastName` та `email` мають тип `String`, поля `age` і `diseaseId` &mdash; тип `Int`, і два поля &mdash; тип `Bool`.
...
Але що ж за інтригу я приготував насамкінець? Попередньо я згадав, що мітки використовуються не тільки для задання значень полів і для їх отримання, але і для зміни. Ось що я мав на увазі:
...
код скомпілюється, але уважний компілятор попередить вас про проблему:
...
З поняттям &laquo;синтаксичний цукор&raquo; ми зустрінемося ще не раз, на більш складних прикладах.
